...
<div class="relative {{ $dropdownContainerClass }}" x-data="{
    open: false,
    value: '{{ $inputValue }}',
    label: '{{ $label }}',
    ...
    x-model="{{ $attributes->whereStartsWith('x-model')->first() }}" x-on:click.outside="setFalse">
    <button class="{{ $buttonClass }} text-left" id="{{ $buttonId }}" x-on:click="toggle" type="button">
        <x-typography :variant="'body-small-regular'"
            x-text="value == '' ? label : Object.entries(options).filter(([key, val]) => val == value).map(([key]) => key)[0]"></x-typography>
        <img src="{{ asset("assets/icons/arrow-down/$arrow-24.svg") }}" alt="Filter"
            class="transition-all duration-200"
            :class="{ '-rotate-180': open && {{ json_encode($isIconCanRotate) }} }">
    </button>
    <div class="dropdown-option dropdown-option-{{ $variant }} {{ $optionStyleClass }}" id="{{ $dropdownId }}"
        @if ($attributes->has('x-data') || $attributes->has('x-init')) x-data="{{ $attributes->get('x-data') }}"
        x-init="{{ $attributes->get('x-init') }}"
      @else
        x-data="{
          options: @js($dropdownItem),
        }" @endif
        x-show.important="open" x-transition
        @if ($attributes->has('x-init')) x-init="{{ $attributes->get('x-init') }}" @endif>
