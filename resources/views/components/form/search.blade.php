...
<x-container.container :background="'content-white'" :radius="'md'" :width="'full'" :height="'max'" :padding="'p-2'"
    class="gap-2" x-data="{
        key: '',
        storeName: '{{ $storeName }}',
        storeKey: '{{ $storeKey }}',
    ...
    x-model="{{ $attributes->whereStartsWith('x-model')->first() }}" x-init="$watch('key', value => onSearch())">
    <template x-if="iconPosition == 'left'">
    ...
    <template x-if="iconPosition == 'right'">
        <div class="flex items-center pointer-events-none text-gray-400">
            <svg xmlns="http://www.w3.org/2000/svg" class="h-5 w-5" fill="none" viewBox="0 0 24 24"
                stroke="currentColor" stroke-width="2">
                <path stroke-linecap="round" stroke-linejoin="round"
