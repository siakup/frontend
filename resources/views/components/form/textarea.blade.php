...
@php
    ...
    $safeRef = 'textarea_' . preg_replace('/[^A-Za-z0-9_\-]/', '_', $name);
    $externalModelExpr = $hasExternalModel ? $attributes->get('x-model') : null;
    $xShowExpression = $externalModelExpr ? $externalModelExpr : ('$refs.' . $safeRef . ' && $refs.' . $safeRef . '.value');
@endphp
...
<x-container.container class="textarea-container state-{{ $finalState }}" w-full>
    ...
    <x-container.container class="textarea-wrapper">
        ...
        @if ($showClearButton)
            <x-button
                ...
                x-show="{!! $xShowExpression !!}"
                x-cloak
                x-on:click="window.clearTextArea($refs.{{ $safeRef }})"
