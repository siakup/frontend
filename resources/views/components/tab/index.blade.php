@props([
    ...
    'widthMax' => false, // true: lebar navbar mengikuti parent // false: sesuai lebar konten
])
...
@php
    ...
    if ($variant === 'underline') {
        ...
        $containerClasses = 'grid grid-flow-col gap-3 mt-4 relative -bottom-px z-20 ' . $containerClass;
        ...
        $containerClasses =
            'grid grid-flow-col gap-0 mt-4 relative -bottom-px z-20 border-b border-gray-300 ' .
            $containerClass;
        $activeClasses =
            $bgActive . ' rounded-t-2.5 ' . $textColorActive . ' font-bold border border-red-200 border-b-0';
        $inactiveClasses = 'text-gray-600 hover:text-red-400';
        $itemClasses = 'py-2.5 px-8 ' . $baseLinkClasses;
    }
@endphp
...
<div class="{{ $containerClasses }}">
    @if (!empty($tabItems))
        @foreach ($tabItems as $tabItem)
            ...
            <a href="{{ route($tabItem->routeName, $tabItem->param ?? []) }}" class="{{ $finalClasses }}">
