...
<script>
  ...
  function onClickSaveSchedule(e) {
    ...
    const newClassSchedule = `<tr class="${Array.from(classScheduleBefore).length % 2 == 0 ? 'bg-white' : 'bg-[#F5F5F5]'} border-b-0">
      <input type="hidden" name="selected_lecture[${Array.from(classScheduleBefore).length}]['hari']" value="${hari}" />
      <input type="hidden" name="selected_lecture[${Array.from(classScheduleBefore).length}]['ruangan']" value="${ruangan}" />
      <input type="hidden" name="selected_lecture[${Array.from(classScheduleBefore).length}]['jam_mulai_kelas']" value="${jamMulai}" />
      <input type="hidden" name="selected_lecture[${Array.from(classScheduleBefore).length}]['jam_akhir_kelas']" value="${jamSelesai}" />
    ...
    document.getElementById('modalAddSchedule').remove();document.getElementById('add-schedule').innerHTML=''
...
<div id="modalAddSchedule" class="modal-custom" style="display:block;">
    ...
    <div class="modal-custom-content !w-[50vw] !min-h-[50vh] h-max overflow-scroll self-center">
        <div class="modal-custom-header">
            ...
            <button type="button" class="modal-close-btn" onclick="document.getElementById('modalAddSchedule').remove();document.getElementById('add-schedule').innerHTML=''">
                &times;
            </button>
        </div>
        <div class="modal-custom-body">
          <div class="form-section">
            ...
            <div class="form-group w-full">
              ...
              <div class="border-[1px] border-[#BFBFBF] rounded-lg flex items-center justify-between px-[12px] py-[9px] text-black" onclick="onClickInputDateTimeInput(this)">
                <input type="text" name="jam_mulai_kelas" class="!bg-transparent !border-none !outline-none !text-black" id="jam-mulai" onfocus="onFocusDateTimeInput(this)" onblur="onBlurDateTimeInput(this)" placeholder="hh:mm">
            ...
            <div class="form-group w-full">
              ...
              <div class="border-[1px] border-[#BFBFBF] rounded-lg flex items-center justify-between px-[12px] py-[9px] text-black" onclick="onClickInputDateTimeInput(this)">
                <input type="text" name="jam_akhir_kelas" class="!bg-transparent !border-none !outline-none !text-black" id="jam-akhir" onfocus="onFocusDateTimeInput(this)" onblur="onBlurDateTimeInput(this)" placeholder="hh:mm">
                <img src="{{ asset('assets/base/icon-calendar.svg') }}" alt="Icon Calendar">
              </div>
            </div>
          </div>
          <div class="button-group self-end">
            <button type="button" class="button button-clean disabled:!bg-white disabled:!border-[#D9D9D9] disabled:!border-1 min-w-[151px] min-h-[40px]" id="btnBatalJadwal" onclick="document.getElementById('modalAddSchedule').remove();document.getElementById('add-schedule').innerHTML=''" disabled>Batal</button>
            <button type="button" class="button button-outline min-w-[151px] min-h-[40px]" id="btnSimpanJadwal" onclick="onClickSaveSchedule(this)" disabled>Tambahkan Jadwal</button>
          </div>
        </div>
    </div>
</div>
