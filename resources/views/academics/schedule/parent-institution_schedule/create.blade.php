...
<script type="module">
  ...
  document.addEventListener('alpine:init', () => {
    ...
    Alpine.data('createParentInstitutionSchedule', ParentInstitutionSchedule.createParentInstitutionSchedule);
    Alpine.data('lectureViewComponents', ParentInstitutionSchedule.lectureViewComponents);
    Alpine.data('courseViewComponents', ParentInstitutionSchedule.courseViewComponents);
    Alpine.data('createScheduleListComponents', ParentInstitutionSchedule.createScheduleListComponents);
  });
</script>
...
@section('content')
  <x-container.container 
    ...
    x-data="createParentInstitutionSchedule()"
  >
    <x-typography :variant="'body-large-semibold'">Tambah Jadwal Kuliah Institusi Parent</x-typography>
        ...
        <x-container.container :variant="'content-wrapper'" :class="'!flex flex-row !p-0 justify-between items-center'">
          ...
          <x-button.primary 
            ...
            x-on:click="showModal(
              '{{ route('academics.schedule.parent-institution-schedule.add-course', ['periode' => '__periode__', 'program_perkuliahan' => '__program_perkuliahan__', 'program_studi' => '__program_studi__']) }}'
    ...
    <x-container.container :class="'flex flex-col !gap-4'">
      <x-container.container :variant="'content-wrapper'" class="!px-0 flex flex-row items-center justify-between">
          ...
          <x-button.primary x-on:click="showModal('{{ route('academics.schedule.parent-institution-schedule.add-lecture') }}', '#list-lecture', '#modalListLecture')">Tambah Pengajar</x-button.primary>
      </x-container>
    ...
    <x-container.container :class="'flex flex-col !gap-4'">
      <x-container.container :variant="'content-wrapper'" class="!px-0 flex flex-row items-center justify-between">
          ...
          <x-button.primary x-on:click="showModal('{{ route('academics.schedule.parent-institution-schedule.add-class-schedule') }}', '#add-schedule', '#modalAddSchedule')">Tambah Jadwal Kelas</x-button.primary>
      </x-container>
    ...
    <x-container.container>
      <x-container.container :variant="'content-wrapper'" class="flex flex-row items-center justify-end !px-0">
          <x-button.secondary :href="route('academics.schedule.parent-institution-schedule.index')" x-bind:disabled="checkValidity">Batal</x-button.secondary>
          <x-button.primary x-on:click="createConfirmationModalOpen = true" x-bind:disabled="checkValidity">Simpan</x-button.primary>
      </x-container>
    </x-container>
    <x-modal.container-pure-js x-bind:class="{'hidden': !createConfirmationModalOpen, 'flex': createConfirmationModalOpen}">
      <x-slot name="header">
      ...
      <x-slot name="footer">
        <x-button.secondary x-on:click="createConfirmationModalOpen = false">Cek Kembali</x-button.secondary>
        <x-button.primary x-on:click="onPostProdiSchedule">Ya, Simpan Sekarang</x-button.primary>
      </x-slot>
