...
@section('content')
    <x-container :variant="'content-wrapper'" x-data="listUser({{ json_encode(route('users.index')) }})">
        ...
        <x-container :class="'flex justify-between'">
            <div class="w-64">
                <x-form.search :value="$search" :placeholder="'Username / Nama / Status'" :storeName="'listPage'" :storeKey="'datas'" :requestRoute="route('users.index')"
                    :responseKeyData="'users'" x-model="$store.listPage.search" />
            </div>
            <x-form.dropdown :buttonId="'sortFilterButton'" :dropdownId="'sortFilterDropdown'" :label="'Urutkan'" :imgSrc="asset('assets/icons/sort/red-20.svg')" :isIconCanRotate="false"
                :dropdownItem="[
                    'Urutkan' => '',
                    'Aktif' => 'active',
                ...
                <template x-if="$store.listPage.datas && $store.listPage.datas.length > 0">
                    <template x-for="user in $store.listPage.datas">
                        <x-table.row :variant="'old'">
                            ...
                            <x-table.cell :variant="'old'" class=" text-gray-12"
                                x-text="formatDateTime(user.created_at)"></x-table.cell>
                            <x-table.cell :variant="'old'">

                                <template x-if="user.status === 'active'"><x-badge :variant="'green-filled'"
                                        x-text="'Aktif'"></x-badge></template>
                                <template x-if="user.status !== 'active'"><x-badge :variant="'green-bordered'"
                                        x-text="'Tidak Aktif'"></x-badge></template>
                            </x-table.cell>
                            <x-table.cell :variant="'old'">
                      ...
                      { nomor_induk: user.nomor_induk }
                                        ...
                                        :variant="'tertiary'" :size="'sm'" class="!text-[#0076BE] text-center">Reset
                                        Password</x-button>
                                </x-container>
                            </x-table.cell>
                            <x-table.cell :variant="'old'">
                                <x-container :variant="'content-wrapper'" :class="'flex flex-row gap-10 items-center justify-center'">
                                    <x-button :variant="'tertiary'" :size="'sm'" :icon="asset('assets/icons/search/black-16.svg')" class="!text-black"
                                        x-on:click="requestDisplayTemplate(
                            '{{ route('users.detail') }}', 
                            '#userDetailModalContainer', 
                            '#modalDetailPengguna', 
