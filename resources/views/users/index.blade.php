...
@section('content')
  <x-container.wrapper :padding="'p-0'" :gapY="4" :rows="12" x-data="listUser({{ json_encode(route('users.index')) }})">
    ...
    <x-container.container :background="'content-white'" :padding="'px-4 py-2'" class="row-start-3 row-end-5 items-center justify-between">
      <div class="w-64">
        <x-form.search
          :value="$search"
          ...
          <template x-if="$store.listPage.datas && $store.listPage.datas.length > 0">
            <template x-for="user in $store.listPage.datas">
              <x-table.row :variant="'old'">
                  ...
                  <x-table.cell :variant="'old'" class=" text-gray-12" x-text="window.formatter.formatDateTime(user.created_at)"></x-table.cell>
    ...
    <template x-if="$store.listPage.datas && $store.listPage.datas.length > 0">
      <x-container.container :background="'transparent'" :width="'full'" :height="'maxContent'" class="'row-start-12 row-end-13">
        <x-pagination 
          x-data="{
            pagination: null,
            requestData: null
