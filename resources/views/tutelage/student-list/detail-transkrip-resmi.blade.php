...
<script type="module">
  ...
  document.addEventListener('alpine:init', () => {
    Alpine.store('detailPage', { 
      ...
      mataKuliahResmi: @js($mataKuliahResmi)
    });

    Alpine.data('detailTranskripResmi', PerwalianKRS.detailTranskripResmi);
  });
</script>
  ...
  >
    ...
    <x-container :variant="'content-wrapper'" :class="'flex flex-col !gap-0 !px-0'">
      ...
      <x-container :class="'flex flex-col gap-4 rounded-tl-none items-stretch my-0 border-t-[1] border-t-[#F39194] relative !z-0'">
        <x-typography variant="body-medium-bold">Transkrip Nilai Resmi</x-typography>
        <x-container :variant="'content-wrapper'" class="!px-0">
          <x-container class="!bg-[#FAFAFA] grid grid-cols-4 !gap-y-3 !gap-x-1 ">
        ...
        <x-container :variant="'content-wrapper'" class="!p-0 !pt-3">
            <x-typography variant="body-medium-bold">Tahap Sarjana</x-typography>
            <x-table class="text-sm text-center">
              <x-table-head>
                <x-table-row>
                  ...
                  <x-table-header>Historis Nilai</x-table-header>
                  <x-table-header>Konversi</x-table-header>
                  <x-table-header>Status</x-table-header>
                </x-table-row>
              </x-table-head>
              <x-table-body>
                <template x-for="(matkul, index) in $store.detailPage.mataKuliahResmi">
                  <x-table-row>
                    ...
                    <x-table-cell x-text="matkul.tahun+'/'+matkul.semester+'/'+matkul.nilai"></x-table-cell>
