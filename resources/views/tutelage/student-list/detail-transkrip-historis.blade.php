...
<script type="module">
  ...
  document.addEventListener('alpine:init', () => {
    Alpine.store('detailPage', { 
      historis: @js($historis),
      transkrip: @js($transkrip)
    });

    Alpine.data('detailTranskripHistoris', PerwalianKRS.detailTranskripHistoris);
  });
</script>
...
@section('content')
  <x-container
    ...
    x-data="detailTranskripHistoris()"
  >
    ...
    <x-container :variant="'content-wrapper'" :class="'flex flex-col !gap-0 !px-0'">
      ...
      <x-container :class="'flex flex-col gap-4 rounded-tl-none items-stretch my-0 border-t-[1] border-t-[#F39194] relative !z-0'">
        <x-typography variant="body-medium-bold">Transkrip Nilai Historis Mahasiswa</x-typography>
        ...
        <template x-for="semesterData in $store.detailPage.historis">
              ...
              <x-table-body>
                ...
                <x-table-row>
                  <x-table-cell colspan="6" class="text-left border-b-0">
                    <x-container class="flex flex-col space-y-1">
