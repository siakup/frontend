...
class ScheduleController extends Controller
{
    ...
    public function index(Request $request)
    {
        try {
            ...
            $dataSchedule = [
                (object) [
                    'id_kelas' => 1,
                    'periode' => current(array_filter($periodeList, function ($item) {
                        return $item->id == 18;
                    }))->tahun.'/'.current(array_filter($periodeList, function ($item) {
                        return $item->id == 18;
                    }))->semester,
    ...
    public function downloadTemplate(Request $request)
    {
        try {
            ...
            return Excel::download(new class($data) implements FromArray, WithCustomCsvSettings, WithHeadings
            {
                private $rows;
