    ...
    public function eventDownloadTemplate(Request $request, $id)
    {
        try {
            ...
            if (! in_array($type, $allowed)) {
                throw new \Exception('Format File yang diminta tidak sesuai!');
            }
            ...
            $programStudiList = $responseProgramStudiList->data;
            $id_prodi = $request->input('program_studi', $programStudiList[0]->id);
            $programStudi = current(array_filter($programStudiList, function ($prodi) use ($id_prodi) {
                return $prodi->id == $id_prodi;
            }));
